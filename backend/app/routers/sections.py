...
from app.models import Section, AcademicYear, User
...
def create_section(section: SectionCreate, db: Session = Depends(get_db), _admin: User = Depends(require_admin)):
    if not db.query(AcademicYear).filter(AcademicYear.id == section.year_id).first():
...
def delete_section(section_id: str, db: Session = Depends(get_db), _admin: User = Depends(require_admin)):
    section = db.query(Section).filter(Section.id == section_id).first()
    if not section:
        raise HTTPException(status_code=404, detail="Section not found")
    db.delete(section)

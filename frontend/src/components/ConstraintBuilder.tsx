...
export function ConstraintBuilder() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingProfile) {
        await constraintsApi.updateProfile(editingProfile.id, formData);
      } else {
  ...
  const handleNLImport = () => {
    if (nlResult && nlResult.validation?.valid !== false) {
      setFormData({
        id: `profile_${Date.now()}`,
        name: 'Imported from NL',
        soft_constraint_weights: {
          ...(nlResult.soft_constraint_weights || {
          minimize_gaps: 5,
          lab_theory_mix: 3,
          teacher_preference: 2,
          empty_day_preference: 2
          }),
          structured_constraints: nlResult.constraints || []
        }
      });
      setShowForm(true);
      setNlResult(null);
          ...
          <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
            ...
            {(nlResult.ambiguities?.length > 0 || nlResult.unsupported_requests?.length > 0) && (
              <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-900">
                <p className="font-medium">Review required before saving</p>
                {nlResult.ambiguities?.map((item: string) => <p key={item}>Ambiguity: {item}</p>)}
                {nlResult.unsupported_requests?.map((item: string) => <p key={item}>Unsupported: {item}</p>)}
              </div>
            )}
            <button
              onClick={handleNLImport}
              disabled={nlResult.validation?.valid === false}
              className="mt-3 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm"
            >
              Import validated constraints as Profile
            </button>
          </div>
        )}
      </div>
        ...
        <div className="mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mb-3">
                These are preferences, not requirements. Use 0 to ignore a preference and 10 to make it very important.
                Start with the balanced values below; hard rules such as room capacity and teacher eligibility always win.
              </p>
